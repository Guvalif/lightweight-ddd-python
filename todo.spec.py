...
class TestTodo(unittest.TestCase):
    ...
    def test_addTag(self):
        todo = todo_module.create_todo('テスト', datetime.now() + timedelta(days=1))
        edited_todo = todo_module.add_tag(todo, 'プライベート')

        self.assertTrue('プライベート' in edited_todo['tags'])
    ...
    def test_removeTag(self):
        ...
        edited_todo = todo_module.remove_tag(added_todo, 'プライベート')

        self.assertFalse('プライベート' in edited_todo['tags'])
        self.assertEqual(todo['tags'], edited_todo['tags'])
